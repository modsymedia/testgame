...
export const PointsEarnedPanel = ({
  ...
  return (
    <div className={`w-full ${className}`}> {/* Apply className here */}
      <PixelatedContainer noPadding>
        <div className="w-full">
          ...
          <div className="bg-[#CADA9B] p-4">
            ...
            <div className="text-lg font-bold font-sk-eliz text-[#304700] mb-4">
              {Math.round(currentPoints)}
            </div>

            {/* Points Rate and Multiplier - Dynamic based on game */}
            <div className="mb-4">
              <div className="flex justify-between text-sm font-sk-eliz text-[#304700] mb-2">
                <span>+ {pointsPerSecond}/sec</span>
                <span>{timeUntilUpdate}s until update</span>
              </div>
              <ProgressSlider progress={progress} />
              <div className="text-right text-xs font-bold font-sk-eliz text-[#304700] mt-1">
            ...
            <div className="flex justify-between mb-6">
              <div>
                <div className="text-xs font-sk-eliz text-[#72795F] mb-1">Current</div>
                <div className="text-sm font-bold font-sk-eliz text-[#304700]">{Math.round(currentPoints)}</div>
              </div>
              <div className="text-right">
                <div className="text-xs font-sk-eliz text-[#72795F] mb-1">Next</div>
                <div className="text-sm font-bold font-sk-eliz text-[#304700]">{Math.round(nextPoints)}</div>
              </div>
            </div>

            {/* Task Rewards Section - Static, doesn't change with game state */}
            <div>
              <div className="text-md font-bold font-sk-eliz text-[#304700] mb-3 border-t-2 border-[#304700] pt-4">Task Rewards</div>
                    ...
                    <div className="bg-[#ebffb7] p-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium font-sk-eliz text-[#304700]">
                            {task.name}
                          </span>
                        </div>
                        <span className="text-xs font-bold font-sk-eliz text-[#304700]">
                          +{task.points}
                        </span>
                      </div>
                    </div>
