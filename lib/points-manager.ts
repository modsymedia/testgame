...
export class PointsManager {
  ...
  public onTransaction(callback: (tx: PointTransaction) => void): () => void {
    ...
    return () => {
      this.transactionCallbacks = this.transactionCallbacks.filter(
        (cb) => cb !== callback
      );
    };
  }
  ...
  private calculateMultiplier(user: User): number {
    ...
    if (consecutiveDays >= this.config.multipliers.consecutiveDaysMin) {
      const daysOverMin =
        consecutiveDays - this.config.multipliers.consecutiveDaysMin;
  ...
  public async awardPoints(
    ...
    try {
      ...
      if (
        (source === "interaction" || source === "gameplay") &&
        this.isOnCooldown(user, source)
      ) {
        return {
          success: false,
          points: 0,
      ...
      if (["gameplay", "interaction", "daily"].includes(source)) {
        if (dailyPoints + pointsToAdd > this.config.caps.dailyPoints) {
          adjustedPoints = Math.max(
            0,
            this.config.caps.dailyPoints - dailyPoints
          );
        }
      ...
      if (source === "interaction") {
        ...
        if (
          interactionPoints + pointsToAdd >
          this.config.caps.interactionPoints
        ) {
          adjustedPoints = Math.max(
            0,
            this.config.caps.interactionPoints - interactionPoints
          );
        }
      }
      ...
      if (source === "interaction" || source === "gameplay") {
        ...
        if (source === "interaction") {
          cooledDownUser.cooldowns = {
            ...cooledDownUser.cooldowns,
            interactionTotal:
              (cooledDownUser.cooldowns?.interactionTotal || 0) +
              adjustedPoints,
          };
        }
  ...
  public async awardAchievement(
    ...
    try {
      ...
      const userAchievements = user.cooldowns?.achievements as
      ...
      if (result.success) {
        ...
        if (updatedUser) {
          const existingAchievements =
            (updatedUser.cooldowns?.achievements as
              | Record<string, number>
              | undefined) || {};
      ...
      try {
        ...
        if (cachedData) {
          ...
          if (parsed.userPointsCache) {
            Object.keys(parsed.userPointsCache).forEach((key) => {
              if (parsed.userPointsCache[key].lastUpdate) {
                parsed.userPointsCache[key].lastUpdate = new Date(
                  parsed.userPointsCache[key].lastUpdate
                );
              }
            });
          ...
          if (parsed.pendingTransactions) {
            this.pendingTransactions = parsed.pendingTransactions.map(
              (tx: any) => ({
                ...tx,
          ...
          if (parsed.transactionHistory) {
            Object.keys(parsed.transactionHistory).forEach((key) => {
              this.transactionHistory.set(
                key,
                (parsed.transactionHistory[key] || []).map((tx: any) => ({
                  ...tx,
                  timestamp: new Date(tx.timestamp),
                }))
              );
